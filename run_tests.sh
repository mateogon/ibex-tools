...
max_jobs=$(nproc) #$(getconf _NPROCESSORS_ONLN) #$(nproc)
# Parameter combinations to test
alpha_values=(0.8 0.75 0.85) #(0.8 0.9 1.0)
max_iter_values=(4 5 6) #(5 10 15)
prec_values=(1e-4 5e-5 2e-4) #(1e-4 1e-3 1e-2)
...
for alpha in "${alpha_values[@]}"; do
    for max_iter in "${max_iter_values[@]}"; do
	    ...
	    tail -1 waf_install.log
            ...
            run_python_script $alpha $max_iter $prec $tools_dir $input_file
            ((loop_number++))
        done
    done
done
